...
import { Camera, RefreshCcw, AlertCircle, ShieldCheck, Album } from "lucide-react";
...
export default function Home() {
    ...
    const handleManualSubmit = async (barcode: string) => {
        ...
        setMode("loading");
        try {
            const response = await fetchProductByBarcode(barcode, allergies);
            if (response.error) {
                setError(response.error || "Food not found");
                setMode("error");
            } else if (response.data) {
        ...
        <main className="relative w-full max-w-md mx-auto px-6 pb-20 flex flex-col items-center min-h-screen justify-center overflow-x-hidden bg-deep-stone text-block-white app-container">
            ...
            <div id="main-content" className="w-full space-y-6 flex flex-col items-center">
                <AnimatePresence mode="wait">
                    ...
                    {mode === "loading" && (
                        ...
                        >
                            ...
                            <p className="mt-8 text-4xl font-display font-bold text-block-white animate-pulse">Looking for clues...</p>
                        </motion.div>
                    )}
                    ...
                    {mode === "result" && result && (
                        <ResultView
                            ...
                            result={result}
                            onReset={reset}
                            onCollect={() => toggleSnack({ barcode, name: result.name, brand: result.brand })}
                    ...
                    {mode === "error" && (
                        ...
                        >
                            <div className="bg-redstone-red p-12 border-4 border-ink-navy text-center shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] w-full">
                                <AlertCircle className="w-28 h-28 text-block-white mb-8 mx-auto" aria-hidden="true" />
                                <h3 className="text-6xl font-display font-black text-block-white uppercase tracking-tighter">Oops!</h3>
                                <p className="text-3xl text-block-white font-body font-bold mt-6">{error}</p>
                            </div>
                            <button
                                onClick={reset}
                ...
                <dialog
                  id="allergy-settings"
                  open={isSettingsOpen}
                  className="rounded-[3rem] border-8 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] p-0 bg-block-white max-w-lg w-full"
